...
1. Create an Azure resource group in your subscription.
   - Use `taw-mca-challenge-XXXXX` as the name of the new resource group, where `XXXXX` is a unique suffix, such as your initials or a random number used to ensure a unique resource group name.
...
In this challenge, you retrieve various connection string values from resources in your `taw-mca-challenge-SUFFIX` resource group and use them to create secrets in Key Vault. Additionally, you will create references to the secrets in Key Vault and add them to the App Configuration resource.
...
1. In the Azure portal, select Resource groups from the left-hand menu, then locate and delete the `taw-mca-challenge-SUFFIX` resource group.
